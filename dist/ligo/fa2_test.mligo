...
let test_mint  =
  ...
  let mi_owners = [(src, (Amount 10n)); (other, (Amount 10n))] in
  let mint_param =
    {
      token_id = (Token_id 12318n);
  ...
  let () =
    assert
      ((Big_map.find_opt (src, (Token_id 12318n)) storage.ledger) =
       (Some (Amount 10n))) in
  let () =
    assert
      ((Big_map.find_opt (src, (Token_id 10n)) storage.ledger) =
       (None : token_amount option)) in
  storage
