...
let next_token_id (storage : storage) = storage.next_token_id
...
let test_mint =
  ...
  let quantity =
    List.fold
      (fun ((acc, (_, am)) : nat * (address * nat)) -> acc + am)
      mi_owners
      0n
  in
  let mint_param =
    {mi_token_id; mi_token_info = (Map.empty : (string, bytes) map); mi_owners}
  in
  ...
  let () =
    assert (Big_map.find_opt (10n, src) storage.ledger = (None : nat option))
  in
  storage
