...
let transfer (txs : transfer list) (s : storage)
    (validate_op : address -> address -> token_id -> storage -> unit) : ledger =
  List.fold
    (fun ((ledger, tx) : ledger * transfer) ->
      List.fold
        (fun ((ledger, dst) : ledger * transfer_destination) ->
          ...
          let () =
            validate_op tx.tr_src (Tezos.sender None) dst.tr_token_id s
          in
...
let default_operator_validator (owner : address) (operator : address)
    (token_id : token_id) (s : storage) : unit =
...
let fa2 ((param, storage) : assets_params * storage) : operation list * storage
  ...
  match param with
  | Transfer txs ->
      let ledger = transfer txs storage default_operator_validator in
