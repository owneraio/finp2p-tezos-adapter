...
let transfer (txs : transfer list) (validate_op : address -> address -> nat -> storage -> unit)
  ...
  List.fold (fun (l, tx : ledger * transfer) ->
      List.fold (fun (ll, dst : ledger * transfer_destination) ->
          match Big_map.find_opt (dst.tr_token_id, tx.tr_src) ll with
          ...
          | Some am ->
            if dst.tr_amount = 0n then ll
              ...
              | None -> (failwith fa2_insufficient_balance : ledger)
              | Some diff ->
                let () = validate_op tx.tr_src (Tezos.sender None) dst.tr_token_id s in
                let ll =
                  if diff = 0n then Big_map.remove (dst.tr_token_id, tx.tr_src) ll
                  else Big_map.update (dst.tr_token_id, tx.tr_src) (Some diff) ll in
                match Big_map.find_opt (dst.tr_token_id, dst.tr_dst) ll with
                | None -> Big_map.add (dst.tr_token_id, dst.tr_dst) dst.tr_amount ll
...
let update_operators_for_all (s : operators_for_all_storage) (l : operator_update_for_all list) : operators_for_all_storage =
  List.fold (fun (s, a : operators_for_all_storage * operator_update_for_all) ->
      ...
      | Remove_operator_for_all op -> Big_map.remove (op, Tezos.sender None) s) l s
